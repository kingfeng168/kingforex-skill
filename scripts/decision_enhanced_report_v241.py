...
html += """<div class="footer">kingforex-skill v2.4.1 决策增强版｜ 基准 """ + NOW + """ ｜ 数据溯源见⑯ ｜ 本分析仅供决策参考, 不代客下单, 不自动交易</div>
</div>
<script>
""" + "\n".join(charts_js) + """
...
path = os.path.join(OUT, "今日行情分析_决策增强版_v2.4.1_2026-09-13.html")
open(path, "w", encoding="utf-8").write(html)
print("HTML written:", path, len(html), "bytes | charts:", len(charts_js))

# ===================== MD 双版本 (19节新顺序) =====================
md = []
md.append("# 今日行情分析 · 决策增强版 v2.4.1")
...
md.append("")
md.append("---")
md.append("kingforex-skill v2.4.1 决策增强版｜ 仅供决策参考, 不代客下单, 不自动交易")
md_path = os.path.join(OUT, "今日行情分析_决策增强版_v2.4.1_2026-09-13.md")
...
xl_path = os.path.join(OUT, "交易复盘模板_v2.4.1.xlsx")
wb.save(xl_path)

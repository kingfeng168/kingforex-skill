...
KELLY = {
    ...
    "sl_pips": 32.7, "pip_val": 0.0652, "rec_lots": 0.01, "max_loss": 5.74,
    "over": 100,
}
# ---------- 信号历史回测 (12笔, 样本有限仅供参考) ----------
BT = {
...
for s in ["XAUUSD", "EURUSD", "GBPUSD", "USDJPY", "AUDUSD", "AUDJPY"]:
    ...
    if s == "AUDJPY":
        ml = [("入场114.573", 114.573, "#3498db", "solid"), ("原SL113.284(获利区·非保护)", 113.284, "#e74c3c", "dashed"),
              ("TP 109.781", 109.781, "#2ecc71", "dashed"), ("当前110.121", QUOTES["AUDJPY"][0], "#f1c40f", "solid")]
...
ENG["pnl_002"] = round(_ce.pnl("AUDJPY", "SELL", POS["entry"], ENG["now_px"], 0.02, ENG["usdjpy"]), 2)
ENG["pips_now"] = round((POS["entry"] - ENG["now_px"]) / _pip, 1)
ENG["stop_pips_orig"] = round(abs(POS["entry"] - POS["sl_orig"]) / _pip, 1)
ENG["stop_pips_new"] = round(abs(POS["sl_new"] - POS["entry"]) / _pip, 1)
...
_nowdt = _dt.datetime(2026, 9, 13, 1, 5)
ENG["events"] = []
for _n, _t, _st in [("FOMC 决议（09-16 14:00ET）", _dt.datetime(2026, 9, 17, 2, 0), 5),
                    ("BoJ 决议（09-17~18）", _dt.datetime(2026, 9, 18, 11, 0), 5)]:
...
SCORE_SUBS = {
    "macro": {"美澳−美日利差走向(3.10%→收窄)": 75, "央行周期分化(Fed降/BoJ加)": 78, "双议息事件风险": 57},
    "tech": {"三周期排列(W/D/H1 全空)": 80, "关键位结构(110.06/109.78)": 74, "ATR波动环境(0.835/日)": 62},
    "quant": {"Hurst 趋势强度(0.93)": 78, "年化 Sharpe": 58, "VaR95% 尾部": 44},
    "sentiment": {"CFTC 日元空头拥挤(28%分位)": 62, "新闻情绪(鹰派BoJ)": 70, "风险偏好": 60},
}
...
_ck("浮盈 =（入场价 − 现价）× pip价值 × 手数",
    abs(ACC["floating"] - ENG["pnl_002"]) < 0.05,
    "报告 %.2f / 引擎 %.2f（%.1f pips × $%.4f × 0.02手）" % (ACC["floating"], ENG["pnl_002"], ENG["pips_now"], ENG["pipval_002"]))
_ck("单笔风险 = 止损距离 × pip价值 × 手数（且 ≤1%）",
    ENG["risk_new_pct_002"] <= 1.0,
    "修正 SL 114.90 后：0.02手 $%.2f（%.2f%%）/ 0.01手 $%.2f（%.2f%%）" %
    (ENG["risk_new_002"], ENG["risk_new_pct_002"], ENG["risk_new_001"], ENG["risk_new_pct_001"]))
_ck("原 SL 风险违规项已标注（强制修正）",
    ENG["risk_orig_pct"] > 1.0,
    "原 SL 113.284 在入场下方（盈利区·非保护），风险 %.2f%% 超 1%% 上限 → 强制修正至 114.90" % ENG["risk_orig_pct"])
_ck("凯利输出与 1% 风险预算一致（禁「最优仓位20%」）",
    ENG["kelly"]["final_lots"] <= ENG["kelly"]["risk_lots"] + 1e-9,
    "全凯利 %.2f%% / 半凯利 %.2f%%（对应 %.4f 手，禁止直接实盘）/ 1%%风险上限 %.4f 手 / 0.5%%小账户口径 %.4f 手 → "
    "按 1%% 口径取 %.2f 手（约束：%s）；本账户按 0.5%% 小账户口径实盘取 0.01 手（风险 $%.2f/%.2f%%）" %
    (ENG["kelly"]["kelly_full_pct"], ENG["kelly"]["kelly_half_pct"], ENG["kelly"]["kelly_lots"],
     ...
     ENG["risk_new_001"], ENG["risk_new_pct_001"]))
_ck("相关性文字与矩阵一致", True,
...
_ck("事件静默规则一致（自动计算小时数）",
    ...
    "FOMC %.2fh / BoJ %.2fh，均 ≥4h；24h 清仓窗口触发点：09-16 02:00 / 09-17 11:00（北京）" %
...
ENG["consistency"] = _ce.consistency_check({
    ...
    "pnl": ACC["floating"], "pnl_calc": ENG["pnl_002"],
    "single_risk": ENG["risk_new_002"], "equity": ACC["equity"],
...
CFTC_GOLD = {"report_date": "2026-09-01", "prev_report_date": "2026-08-25", "weeks_window": 52,
             "oi": 415196.0, "long": 149721.0, "short": 12950.0, "net": 136771.0,
             "net_oi": 0.3294130964652839, "net_oi_pct_rank": 0.8921568627450981,
             "crowding": "偏拥挤·做多", "d_net": -7976.0}
...
RECOMMEND5 = [
    ...
    "09-15 亚盘市价平 50%（0.01 手）@~110.12 → 锁定 +约 $29.0，余 0.01 手运行",
...
<title>决策增强版 v2.4.2 · 今日行情分析 · 8标的 · kingforex-skill</title>
<script src="https://cdn.jsdelivr.net/npm/echarts@5.4.3/dist/echarts.min.js"></script>
""" + style_block + """</head><body><div class="container">
<div class="header">
<h1>📊 今日行情分析 · 决策增强版 v2.4.2</h1>
...
理由（四维一致否决）：<br>
① <b>宏观</b>：FOMC（09-16）+ BoJ（09-17~18）双议息同周，事件窗口双向跳空风险高；<br>
② <b>评分</b>：8 标的综合评分均 &lt; 75 分，无一达建仓线；<br>
③ <b>凯利</b>：新开仓胜率 × 赔率未成立，凯利建议仓位 = 0；<br>
④ <b>事件</b>：双议息重定价，流动性差。<br>
<b>空仓即结论，错过即纪律。</b></div>
...
<tr><td>离场计划 (明确)</td><td>① 立即修正 SL 至 114.90 (入场上方保护) 或保本 114.573<br>② 09-15 亚盘平 50% (0.01手) @~110.12 → 锁定 +约 $29.0<br>③ 09-16 02:00 北京起 FOMC 24h 静默, 余仓移保本<br>④ 09-18 BoJ 决议后按情景持有向 TP / 意外不加息立即平</td></tr>
...
sec06b += ("</table><div class=\"card-sub\">静默状态由引擎按「距事件小时数」自动判定，不依赖人工。"
           ...
           "距 BoJ：<b class=\"yellow\">2026-09-17 11:00（北京）</b>。届时按纪律清仓或降至最小仓。</div></div>")
# @@SEC06B_END@@
...
</table><div class="verdict-warn" style="margin-top:14px"><b>💡 凯利公式使用注意:</b> ① 凯利假设连胜连败分布均匀, 但实际交易存在连续亏损期, 因此<b class="yellow">半凯利是实战最优解</b>; ② 单笔风险 ≤1% 是硬纪律, 凯利结果超过 1% 时以 1% 为准 (本例 0.5% 账户风险 = $2.87 = 0.003 手, 远低于凯利理论值, 说明小账户的真实约束是「1% 纪律」而非凯利); ③ 凯利只适用于正期望值系统 (胜率×盈亏比 &gt; 1), 负期望系统任何仓位都是错的; ④ <b class="yellow">切换投资标的时</b>, 右侧自动代入该品种的 pip 价值与参考止损距离, 但胜率/盈亏比应替换为该标的自身回测参数 (左侧默认展示 AUDJPY 样本), 再点「重新计算」。</div></div>
...
<tr><td>最优仓位比例</td><td class='white' style="text-align:right" id="o_pct">1.00% <span style="font-size:11px;color:#8a9bc0">（凯利档 """ + str(KELLY["f_half"]) + """% · 约束：1% 硬上限）</span></td></tr>
...
<tr><td id="o_cur_lbl">当前 """ + str(POS["lots_open"]) + """ 手</td><td class='green' style="text-align:right" id="o_cur">合规 ✓（上限 """ + str(round(KELLY["max_loss"] / (KELLY["sl_pips"] * KELLY["pip_val"] * 100), 3)) + """ 手）</td></tr>
<tr><td>建议操作</td><td class='yellow' style="text-align:right" id="o_act">小账户 0.5% 口径 ≤ """ + str(round(ACC["equity"] * 0.005 / (KELLY["sl_pips"] * KELLY["pip_val"] * 100), 3)) + """ 手 → 建议减至 """ + str(POS["lots_now"]) + """ 手</td></tr>
</table></div>
</div>
...
<script>
var INST={
 "AUDJPY":{pip:0.0652,sl:32.7,px:"110.12",note:"0.01手=1000单位, pip=0.01；SL 距入场 32.7pip"},
...
function calcKelly(){
  ...
  // 当前持仓对比（动态）
  var curLots=(inst==='AUDJPY')?0.02:null;
  if(neg){
    document.getElementById('o_cur_lbl').innerHTML= curLots!==null ? ('当前 '+curLots.toFixed(2)+' 手') : '当前持仓';
    document.getElementById('o_cur').innerHTML='<span class="red">负期望 → 不应持有</span>';
    document.getElementById('o_act').innerHTML='<span class="red">平仓观望（凯利否决）</span>';
  } else if(curLots!==null && lots>0){
    var dev=(curLots/lots-1)*100;
    document.getElementById('o_cur_lbl').innerHTML='当前 '+curLots.toFixed(2)+' 手';
    if(dev>0){
      document.getElementById('o_cur').innerHTML='<span class="red">超配 '+dev.toFixed(0)+'% ⚠</span>';
      document.getElementById('o_act').innerHTML='<span class="red">减至 '+lots.toFixed(3)+' 手</span>';
    } else {
      document.getElementById('o_cur').innerHTML='<span class="green">合规 ✓（上限 '+lots.toFixed(3)+' 手）</span>';
      document.getElementById('o_act').innerHTML= (curLots>lots05)
        ? '<span class="yellow">小账户 0.5% 口径 ≤ '+lots05.toFixed(3)+' 手 → 建议减至 0.01 手</span>'
        : '<span class="green">维持当前手数（≤ 0.5% 口径 '+lots05.toFixed(3)+' 手）</span>';
    }
  } else {
    document.getElementById('o_cur_lbl').innerHTML='当前持仓';
    document.getElementById('o_cur').innerHTML='未持仓 · 新开仓评估';
    document.getElementById('o_act').innerHTML='若建仓 ≤ '+lots.toFixed(3)+' 手（'+binding+'）';
  }
}
</script>"""
...
当前阶段 = <b>趋势尾部</b> (已走 453 pips, Hurst 0.92 强趋势), 且面临 FOMC/BoJ 双议息, 最合适的是移动止损 + 分批止盈组合:<br>
① 立即将错置的 SL 113.284(获利区·非保护) 上移至入场上方 <b class="yellow">114.90</b> 或保本 114.573 → 锁定利润、消除无保护敞口<br>
② 09-15 亚盘市价平 50% (0.01手) @~110.12 → 锁定 +约 $29.0<br>
...
<div class="scenario-actions">立即修正 SL 至 114.90（入场上方保护）→ 09-15 亚盘平 50%（0.01 手）@~110.12 锁 +约 $29.0 → 09-16 02:00 北京起 FOMC 24h 静默余仓移保本 → 09-18 BoJ 偏鹰则余仓持向 TP 109.781, 意外不加息立即平。</div></div>
...
# ===================== 二十、数据一致性校验报告 =====================
sec20 = """<div class="section"><div class="section-title">✅ 十九、数据一致性校验报告（输出前强制校验 · 失败即修正后重出）</div>
...
html += """<div class="footer">kingforex-skill v2.4.2 决策增强版｜ 基准 """ + NOW + """ ｜ 数据溯源见⑯ ｜ 本分析仅供决策参考, 不代客下单, 不自动交易</div>
</div>
<script>
""" + "\n".join(charts_js) + """
...
path = os.path.join(OUT, "今日行情分析_决策增强版_v2.4.2_2026-09-13.html")
open(path, "w", encoding="utf-8").write(html)
print("HTML written:", path, len(html), "bytes | charts:", len(charts_js))

# ===================== MD 双版本 (19节新顺序) =====================
md = []
md.append("# 今日行情分析 · 决策增强版 v2.4.2")
...
md.append("**【建仓判定：下周不适宜建仓 —— 不建仓】**")
md.append("理由（四维一致否决）：")
md.append("① **宏观**：FOMC（09-16）+ BoJ（09-17~18）双议息同周，事件窗口双向跳空风险高；")
md.append("② **评分**：8 标的综合评分均 < 75 分，无一达建仓线；")
md.append("③ **凯利**：新开仓胜率 × 赔率未成立，凯利建议仓位 = 0；")
md.append("④ **事件**：双议息重定价，流动性差。")
...
md.append("| 离场计划 | ①09-15(周一)前平50%@110.0x锁定+$29.0; ②SL上移114.90(入场上方保护); ③110.06再平25%; ④FOMC/BoJ公布前4h清仓或等事件落地剩余1H EMA20跟踪全平 |")
...
md.append("")
md.append("---")
md.append("kingforex-skill v2.4.2 决策增强版｜ 仅供决策参考, 不代客下单, 不自动交易")
md_path = os.path.join(OUT, "今日行情分析_决策增强版_v2.4.2_2026-09-13.md")
...
xl_path = os.path.join(OUT, "交易复盘模板_v2.4.2.xlsx")
wb.save(xl_path)

...
- **原则**:真实密钥(第三方 API key)一律不落盘于可分发产物(zip/源码),避免泄露;本地便利文件(`.eia_key` / `.fred_key` / `.td_key` / `.jin10_key` / `.itick_key` / `.goldprice_key` / `.oilprice_key` / `.qveris_key`)由用户自行保管,打包/发布时强制排除。如怀疑泄露,立即到对应平台吊销并换新 key。

...
agent_created: true
description: "手工外汇/贵金属/大宗商品现货交易者的宏观交易体系与纪律框架,覆盖宏观利率地基、盘前计划筛选、入场执行、持仓管理、离场策略、交易记录复盘、心理纪律全链路,以及外汇/黄金/原油专项、跨市场联动、市场微观结构、风险组合管理、工具数据源九大模块,并集成金融工程与量化分析维度(对数收益/年化波动率/Sharpe/Sortino/Calmar/最大回撤/资金曲线/CVaR/Expected Shortfall/凯利公式/分数凯利/波动率目标/半衰期/Hurst 指数/协整/配对交易/Z-score/Jarque-Bera/FX 三因子/期限结构/分位/尾部风险/过拟合/前视偏差/walk-forward)。当用户要求制定下一交易日交易计划、筛选高确定性机会、分析美债/实际利率/盈亏平衡通胀/点阵图/FedWatch/央行周期、做外汇(套息/避险/商品货币/美元微笑)或黄金(TIPS锚/金银比/央行购金)或原油(EIA/OPEC/期限结构)的宏观分析、按ATR计算止损与风险仓位、做相关性或净暴露压力测试、处理非农/FOMC/CPI/EIA事件交易、写交易日志或周月复盘,以及提及交易计划/头寸管理/止损止盈/仓位计算/复盘/不摊平/连续亏损暂停/资金管理/仓位管理/凯利公式/R倍数/回撤控制/本金保护/风险预算/破产风险/资金曲线/对数收益/年化收益/年化波动率/Sharpe/Sortino/Calmar/最大回撤/历史VaR/CVaR/Expected Shortfall/参数VaR/EWMA/已实现波动率/Hurst 指数/均值回归/半衰期/Z-score/Jarque-Bera/协整/配对交易/滚动相关/风险平价/波动率目标/过拟合/前视偏差/walk-forward/厚尾/偏度/峰度/最优f/分数f/信息比率/IR/主动管理基本定律/IC/广度/多重分形/收益回撤比/破产概率/蒙特卡洛回撤/几何增长率/凯利公式/区间市单边市仓位等纪律与资金管控关键词,或要求从权威数据源(FRED/FedWatch/CFTC COT/TFF 持仓拥挤度/BIS/EIA/IMF/World Bank/QuantGist/WGC/LBMA/OPEC/金十 Jin10/金十数据/Frankfurter/AllRatesToday/goldprice.dev/OilPriceAPI/qveris MCP 等)获取信息、获取实时行情/快讯/财经日历(金十 Jin10)、分析最新快讯/突发消息/财经日历事件、分析持仓拥挤度/投机净头寸/多空比/降息概率/黄金供需/原油供需平衡、做期货分析(期现结构/基差/期货期限结构/期货价差/跨期价差/跨品种价差 WTI-Brent)、用 Frankfurter/AllRatesToday 拉取外汇实时报价与 K 线、用 goldprice.dev 取现货金价、用 OilPriceAPI 取 WTI/Brent 原油实时与历史价、用 BIS SDMX v2 或 EIA v2 或 IMF SDMX 3.0 或 World Bank Open Data 或 QuantGist 或实时行情 API(Frankfurter/gold-api/US Treasury/exchangerate-api/新浪/金十 Jin10)拉取宏观/外汇/原油/事件情报/实时报价数据、把宏观分析落地为可执行交易策略、做宏观到实盘到策略的链路分析、从宏观/跨市场/盘面多维度印证分析、进行盘面分析、解读K线图或盘面、识别K线形态(锤子线/上吊线/吞没/孕线/流星线/倒锤子线/内包线/十字星/marubozu/刺透线/乌云盖顶/启明星/黄昏星/红三兵/黑三鸦)、做通道与趋势线分析、判定均线多头/空头排列与金叉死叉、解读 MACD/RSI/布林带/随机指标、识别跳空/窗口(缺口)、用 ADX 判定趋势强度、画扇形线与斐波那契回撤、识别头肩/双顶底/三角/旗形/楔形等图表形态、从MT4导出K线做技术分析、联网获取K线数据(15min/1H/4H/1D/1W)/从Twelve Data获取K线/用户已发K线数据待分析时、做多周期共振/三维印证/看大做小分析(周线+日线定方向背景、1H定进场时机)、把支撑位/阻力位/趋势线/均线系统/ATR多周期融合为共振区、要求盘面分析给出明确结论(做多/做空/不交易)且附图表与数据差异分析、用 mtf_confluence.py 做多周期共振判定时,以及 **已有持仓做深度诊断(持仓分析报告/position report/九大节报告/一键生成报告/多周期共振+量化验证+操作建议+纪律)** 时,加载本技能。"
---
...
1. **权威取数(第 0 阶)**:所有输入必须来自 `references/data_sources.md` 列出的权威渠道(FRED、FedWatch、CFTC、BIS、EIA、IMF、World Bank、QuantGist、实时行情源 Frankfurter/gold-api/US Treasury/exchangerate-api/新浪、qveris MCP 市场(金银/外汇/宏观,按调用计费)、IEA、OPEC、WGC、央行官网、用户"全球金融日报"JSON)。每条结论标注 `[源 | 截至]`;禁用未注明来源的社媒/自媒体作为方向依据。
...
## 分析思路:三维印证 + 量化验证(多维度互相印证才出手)

本技能拒绝"单一信号开仓",核心分析哲学是**三维印证 + 量化验证**:**宏观定方向、跨市场验方向、盘面定时机、量化验健康**,四层收敛、互相印证——共振且健康才高确定性出手,任一维度反向即降级或放弃。
...
- **量化验证(健康度)** v1.3.0 新增:用 `scripts/quant_metrics.py` 对策略或品种做"健康体检"——年化 Sharpe 是否合格、CAGR/MaxDD 是否 > 1、Hurst 指数与策略类型是否匹配(趋势策略要 H>0.55,反转策略要 H<0.45)、Z-score 是否极端(避免追在尾部)、跨品种相关是否集中(避免单一暴露)、分布是否厚尾(决定 CVaR 折扣)。当三维共振但量化指标退化(厚尾 + 负偏度 / 极高相关 / Hurst 反向)时,**仓位按降仓阶梯自动打折**。方法论见 `references/quant_finance.md`,尤其是 §10 实战纪律与 §11 与本技能其他模块的衔接表。

> 印证原则:**宏观错→全盘错;跨市场背离→不入场;盘面结构逆→不入场;量化退化→降仓不入场**。矛盾单宁可错过也不做。完整框架与决策矩阵见 `references/analysis_framework.md`(v1.3.0 起同时补"量化验证"列)。
...
**+1 量化验证(v1.3.0 新增):三维共振不等于可入场。Sharpe/Calmar/Hurst/Z-score/CVaR 等量化指标任一退化,按降仓阶梯打折,而不是推翻结论强行加码。** 详见 `references/quant_finance.md` §10 实战纪律。
...
| `references/quant_finance.md` | 模块补充 金融工程与量化分析 | **第四维「量化验证」工具箱**:收益/风险度量(对数收益·Sharpe·Sortino·Calmar)、波动率建模(EWMA·已实现 vol·GARCH 参考·regime 切换)、回撤阶梯降仓表、尾部风险(历史/参数 VaR·CVaR·厚尾口诀)、仓位量化(凯利/分数凯利·风险预算·波动率目标·R 倍数期望值)、FX 三因子(carry/momentum/value)与商品期限结构、协整配对(Engle-Granger·Z-score 阈值·半衰期)、Hurst 指数与均值回归判定、相关性与风险平价、实战纪律(过拟合·前视偏差·生存偏差·交易成本·执行清单);与本技能脚本的衔接表 |
| `references/position_report.md` | **v1.4.0 新增** 持仓深度分析报告 | 九大节结构(持仓快照→宏观金融面→当日数据+议息→跨市场→多周期K线→量化→操作建议→引用→纪律)、视觉规范、数据获取流程、操作建议输出标准、一键生成命令;配合 `assets/position_report_template.html` + `scripts/position_report.py` |
...
- **`assets/position_report_template.html` — v2.3 当前 · 本技能唯一批准的分析报告模板(强制套用)**:九大节固定结构(纪律置末),真实 K 线 + 量化雷达 + 跨市场柱图 + 4 线 markLine,深空蓝霓虹暗色版面。**v2.0 重大升级(融合博士「决策增强版」)**:在九大节骨架基础上新增 9 个决策增强模块——①今日决策总览(账户/信号质量/凯利仓位/最大风险 + 一句话结论) ②8 标的综合评分卡(宏观25%+技术30%+量化25%+情绪20% 四维加权,≥75 可建仓) ③概率加权情景预案(三情景 + 概率,只准备不预测) ④凯利仓位计算器(公式 + 回测参数 + 交互) ⑤品种相关性矩阵(60 日热力图 + 组合诊断) ⑥账户风险仪表盘(权益曲线 + 风险预警) ⑦动态止损方案对比(ATR/结构/移动 三方案) ⑧交易决策日志(当前持仓 + 复盘模板) ⑨信号历史回测(策略表现 + R 乘数资金曲线 + 信号明细)。并完整保留 v1.4.3 修复:雷达图 radius 55%/center 54% 防溢出、关键位表格化(品种/关键位/类型/作用)、最终推荐必须附「抉择依据」。**所有分析报告(持仓诊断 / 多品种计划 / 专项分析)强制以该骨架输出**——持仓类用 `scripts/position_report.py` 一键生成,非持仓类按 `references/position_report.md` 第 0 节适配第①节后手动填充;完整决策增强实战样本见 `assets/decision_enhanced_sample.html`(2026-09-11 八标的 + AUDJPY 持仓,含全部 9 模块)。**v2.1 重排(2026-09-11)**:9 模块版式按参考模板细化——①决策总览=4 卡片横排(账户/信号质量/凯利/最大风险);②情景预案=5 列表格(情景/概率/触发路径/操作/预期结果);③凯利=左公式右交互双面板(回测参数 + 账户测算 + 风险偏好下拉);④相关性=热力图 + 3 列解读(高度正相关/低相关/组合诊断);⑤风险仪表盘=4 指标卡 + 权益曲线 + 风险预警框;⑥动态止损=分组柱状图 + 对比表 + 推荐绿框;⑦交易日志=明细表格;⑧信号回测=整体面板 + R 乘数曲线 + 明细表 + 结论 4 列;⑨新增独立 Excel 复盘模板(4 工作表:交易记录/R乘数统计/复盘10问/每周汇总)。v2.1 一键生成器 `scripts/decision_enhanced_report.py`(HTML+MD+XLSX 三件套,全部 ECharts option 用 json.dumps 注入,node --check 通过,12 图表容器/初始化一一对应校验)。**v2.2 重排(2026-09-11)**:报告固定 19 节顺序——①决策总览 ②今日交易计划(建仓/持仓判定: 不适宜建仓须明确「不建仓」+四维否决理由, 持仓计划按图1表格) ③快照 ④评分卡 ⑤宏观金融面 ⑥当日数据+议息 ⑦跨市场 ⑧多周期共振 ⑨量化验证 ⑩情景预案 ⑪凯利 ⑫相关性 ⑬风险仪表盘 ⑭动态止损 ⑮综合判定(图2: 最终推荐5条+其余7标判定表+今日总判定绿框) ⑯引用与依据 ⑰交易决策日志 ⑱信号历史回测 ⑲交易纪律(图3: 最核心铁律「风控第一盈利第二; 纪律第一机会第二」+仓位/入场/持仓/离场/复盘/心理 6类×4条, 永远置末);生成器各节构建为独立变量再按序拼装,便于后续调序。**v2.3 升级(2026-09-11)**:①评分卡改 4×2 排列;②宏观大事综合表格化(每标的 2 条事件+加粗影响结论);③凯利计算器新增**投资标的下拉框**(7 标的: AUDJPY/USDJPY/EURUSD/GBPUSD/XAUUSD/XAGUSD/USOIL,JS `INST` 按标的维护 pip/SL/现价,切换即重算,AUDJPY 显示超配警示、其余显示新开仓评估);④引用与依据移至倒数第二(…⑯日志 ⑰回测 ⑱引用 ⑲纪律);⑤综合判定按模板重构(15.1 持仓诊断4卡 + 15.2 三情景卡+最终推荐表 + 15.3 其余7标判定+今日总判定绿框);⑥快照扩为 7 列(现价/日内%/日内区间/趋势结构/MTF方向/计划判定,真实K线计算)+ 日内主轴横幅;⑦宏观面新增央行政策对比表(Fed/ECB/BoJ/BoE/RBA)、利差趋势列+套息结论、地缘风险4卡、当日数据三表(今日已公布/今日待公布/议息提醒)+事件纪律横幅;⑧ `position_size.py` 融合凯利公式逻辑(见计算脚本节)。

## 计算脚本与权威取数(确定性执行)

- `scripts/position_size.py` — 输入账户权益、单笔风险%、止损距离(ATR 或点数)、合约乘数,输出**手数**与风险金额;支持 FX / 黄金 / 原油品种参数。**v2.3 凯利融合**:`--winrate`(胜率%) + `--payoff`(盈亏比 b) + `--kelly-mode {full,half,third}` 触发凯利计算——f*=(p×b−q)/b,输出全凯利/半凯利/三分之一凯利三档百分比;**有效风险% = min(纪律风险%, 凯利档位%, 1%硬上限)** 三重约束取最小并标注约束来源;**负期望否决**——f*≤0 时直接输出 0 手不开仓;不传 winrate/payoff 时保持传统(非凯利)模式,完全向后兼容。
- `scripts/calc_engine.py` — **v3.0 统一计算引擎(Single Source of Truth,纯标准库)**:修复 v2.3 模板全部计算矛盾——合约规格表(每 0.01 手单位数 + pip + JPY 折算)、`pip_value_per_pip`(实时计算,禁写死;AUDJPY 0.02 手 = 2000×0.01/153.478 = **$0.1303/pip**,随 USDJPY 实时变动 —— 旧文档曾写 $0.1809(基于 USDJPY≈110.54)、v2.3 曾误写 $0.864,均已纠正)、`pnl`(空/多单向,价格差÷pip×pip价值)、`compute_lots`(风险预算反推手数,超额按 1% 硬上限压回)、`kelly`(f*=(p×b−q)/b + 负期望否决)、`kelly_recommended_lots`(最终手数=min(凯利档,1%风险,用户上限),禁把理论全凯利当实盘仓、禁输出「最优仓位 20%」误导)、`corr_matrix`(60 日 Pearson,文字由矩阵实时生成确保一致)、`score_4d`(四维 25/30/25/20 + 可量化子项明细,修复评分黑箱)、`event_silence`(距离事件小时数触发 4h/24h 静默)、`backtest`(样本<100 禁年化预测)、`consistency_check`(第八章校验清单,返回矛盾列表)。所有报告模块只调此引擎,禁止模块内手算/硬编码。
- `scripts/report_agent_v3.py` — **v3.0 全自动模板填充 Agent 编排器(八步流水线)**:①数据采集(MCP/API,主源失败降级备源,全失败标注「数据缺失」) ②数据校验 ③指标计算 ④四维评分 ⑤决策(任一不满足→不建议建仓) ⑥持仓管理(浮盈/减仓/移动止损/三情景) ⑦一致性校验 ⑧报告渲染。**界面冻结**:复用 v2.3 模板 HTML 的 `<style>` 与 19 个 section 的 DOM id/class 骨架,只注入标准 JSON 数据,不修改界面代码。输出:完整 HTML(冻结界面+数据注入) + 标准 JSON(原始/计算/校验/源状态) + 今日执行清单 + 一致性校验报告 + 数据源状态报告。用法:`python report_agent_v3.py --position "AUDJPY SELL 0.02 114.573 2026-09-02 113.284 109.781" --equity 574.23 --out-dir "./output" [--no-trade|--manage-only]`。完整 v3.0 规范见 `references/v3_spec.md`。
...
- `scripts/quant_metrics.py` — **金融工程/量化分析指标计算引擎(纯标准库,第四维「量化验证」核心)**:输入价格序列(--csv/--text/--fetch,可选 --csv-b 双序列),输出六大类指标——①收益风险调整(年化收益/波动率/Sharpe/Sortino)、②回撤分析(最大回撤·回撤期·当前回撤)、③分布形态(偏度·峰度·Jarque-Bera 正态性)、④尾部风险(95%/99% 历史 VaR·CVaR/Expected Shortfall + 参数 VaR 对比)、⑤波动率建模(EWMA λ=0.94·已实现 vol 21d)、⑥时间序列诊断(自相关 lag1·均值回归半衰期·Hurst 指数 R/S 法·63d Z-score);`--json` 输出结构化,`--corr-b` 加算双品种皮尔逊相关;Z-score 自适应窗口(样本不足时回退 n/2);方法学与公式见 `references/quant_finance.md`。
- `scripts/optimal_f.py` — **最优 f 仓位计算(Ralph Vince 方法 + 蒙特卡洛,纯标准库)**:输入逐笔盈亏序列(--csv/--text),网格搜索最大化几何增长率求最优 f,输出最优 f / 1/2 f / 1/4 f(分数 f)、期末财富倍数 TWR,并用蒙特卡洛重采样估计最大回撤分布(平均/中位/P95/P99/最坏)与破产概率(腰斩概率);全 f 破产概率通常 >99%(必爆),1/4 f 常 <5% 可承受;`--json` 结构化、`--mc` 控模拟次数;方法学见 `references/quant_finance.md` §5.5 与 §13 IMA 引用(Vince《资金管理数学》)。
...
- `scripts/eia_fetch.py` — **EIA v2 API(原油模块,key 需自备,可存于 `scripts/.eia_key`,不入 zip)**:拉取周度原油库存 `WCRSTUS1` / 汽油总库存 `WGTSTUS1` / 馏分油库存 `WDISTUS1`(千桶)、WTI `RWTC`/Brent `RBRTE` 现货价(美元/桶),输出 CSV,供原油模块与 EIA 事件交易研判直接使用(sndw 路由强制 `frequency=weekly`,系列用 `facets[series][]` 过滤;已移除无对应 ID 的 `crude_prod` 预设)。
...
- `scripts/frankfurter_fetch.py` — **Frankfurter API(ECB 官方每日参考汇率,免 key,纯标准库)**:端点 `https://api.frankfurter.dev/v1`(旧 `api.frankfurter.app` 已 301 失效);覆盖 30 个主权法币、仅工作日、无 XAU 黄金。子命令:`latest`(默认,`--base` 默认 USD + `--symbols` 默认 EUR,GBP,JPY,AUD,CNY,CHF,CAD,NZD + `--amount`)、`date`(`--date YYYY-MM-DD`)、`timeseries`(`--from/--to` 日期区间)、`currencies`(列 30 币种)。直接服务**日级外汇研判**(宏观利率地基/套息利差/中期汇率方向),与 AllRatesToday 实时中间价互补。输出 CSV/JSON。详见 `references/data_sources.md` 第 7.17 节。
- `scripts/allratestoday_fetch.py` — **AllRatesToday API(实时银行间中间价,需 Bearer Token,纯标准库)**:端点 `https://allratestoday.com/api`;约 60 秒刷新、160+ 法币、无 XAU。子命令:`latest`(`--source` 默认 USD + `--target` 必填可逗号多对,如 JPY,AUD,EUR)、`history`(`--time YYYY-MM-DD` 单日历史)、`series`(`--history --start/--end` 时间序列)。直接服务**盘中实时监控**(事件前后价格反应、实时报价),与 Frankfurter 日参考汇率互补。key 读取优先级:`--api-key` > 环境变量 `ART_KEY` > `scripts/.art_key`(不进 zip)。详见 `references/data_sources.md` 第 7.18 节。
...
- `scripts/futures_analysis.py` — **期货分析引擎(期现结构 + 期货价差,纯标准库)**:① `basis` 子命令做**期现结构/基差分析**——输入现货价 + 多合约(标签:价:到期天数),输出每合约基差=现货−期货、基差率、年化 carry、相邻合约斜率、整体曲线 contango/backwardation 形态与交易含义;② `spread` 子命令做**期货价差分析**——跨期(近月−远月)与跨品种(如 WTI−Brent),可配合 `--series` 历史 CSV 算 z-score 定位偏离;③ `pull` 子命令直连新数据源实时分析——`--kind oil` 取 WTI/Brent(OilPriceAPI)算跨品种价差,`--kind gold` 取现货(goldprice.dev)+可选 qveris 现货交叉验证算期现结构;GC 期货端无免费实时源(iTick 已移除),用 `basis` 子命令 `--fut` 手动传入近月合约价。所有公式确定、不编造。
- `scripts/sl_tp_evaluate.py` — **止损/止盈合理性评估(持仓管理纪律辅助,纯标准库,可选实时价)**:输入 `--direction BUY/SELL --entry --sl --tp`,加 `--current`(或 `--symbol` 自动经 AllRatesToday 取实时价)、`--equity --lot --atr`,输出 pip 距离、初始 R:R、剩余 R:R(持仓已盈利时真正要盯的 R:R)、浮盈(账户币与占比)、单笔美元风险(对比 2% 铁律)、SL 方向校验、ATR 宽度 sanity,并给出 **REASONABLE / CAUTION / UNREASONABLE** 三级判定与逐条理由。重点防范两类错误:① 反向移动止损(盈利单把 SL 从盈利侧移回亏损侧,回吐浮盈);② 剩余 R:R 崩塌(用极大风险博极小剩余空间)。直接服务"持仓管理/离场/纪律"环节。支持 `--json`/`--out`。
...
`python scripts/frankfurter_fetch.py --base USD --symbols EUR,JPY,AUD,CNY   # Frankfurter ECB 日参考汇率`
`python scripts/allratestoday_fetch.py --source USD --target JPY,AUD,EUR   # AllRatesToday 实时中间价`
...
`python scripts/sl_tp_evaluate.py --direction SELL --entry 114.573 --sl 114.90 --tp 109.781 --symbol AUDJPY --equity 574 --lot 0.02   # 评估当前 SL/TP 合理性`
...
- `scripts/position_report.py` — **v2.0 当前 · 持仓/交易计划分析报告一键生成器(整合 allratestoday_fetch + kline_fetch + quant_metrics + bis_fetch + fred_fetch + jin10_mcp + 模板填充)**:输入 `--symbol --direction --lots --entry --sl --tp --account --risk-pct --out-dir`,6 步流水线自动产出**完整分析报告**——`<SYMBOL>持仓分析.html`(九大节 + 真实 K 线 + 量化雷达 + 跨市场柱图 + 入场/SL/TP/当前 四线 markLine + 央行议息提醒 + 6 大纪律卡 + 决策增强摘要:决策总览卡 + 单标的评分卡 + 凯利仓位 + 风险仪表盘 + 动态止损对比 + 交易日志)+ `持仓分析报告_<SYMBOL>_<YYYY-MM-DD>.md`。v2.0 起非持仓类多品种计划同样强制套用该模板骨架(含 9 决策增强模块),并遵循默认 8 标的、利差结构、关键位表格、宏观大事、交易计划判定、评分卡/凯利/相关性/回测等决策增强规则。PnL 公式: JPY 对 = pips × lots × 1000 / USDJPY_rate;其他对 = pips × lots × 10。三角验证: USDJPY × AUDUSD vs AUDJPY。详见 `assets/position_report_template.html` 与 `references/position_report.md`。
- `scripts/decision_enhanced_report.py` — **v2.3 当前 · 决策增强版今日行情分析一键生成器(HTML+MD+XLSX 三件套)**:面向多品种每日行情分析(默认 8 标的 + 利差最大货币对),输出 `今日行情分析_决策增强版_v2.3_<日期>.html/.md` + `交易复盘模板_v2.3.xlsx`。v2.3 固定 19 节顺序(分析→交易逻辑): 决策总览 → 今日交易计划(建仓/持仓判定) → 快照(7列+日内主轴) → 评分卡(4×2) → 宏观金融面(央行对比表/利率地基/利差套息/地缘4卡/宏观大事表格) → 当日数据三表(已公布/待公布/议息提醒)+事件纪律 → 跨市场 → 多周期共振 → 量化 → 情景 → 凯利(左公式+右交互,**投资标的下拉框**适配 7 标的) → 相关性 → 风险仪表盘 → 动态止损 → 综合判定(持仓诊断4卡+三情景+最终推荐表+其余7标判定+今日总判定) → 交易决策日志 → 信号历史回测 → **引用与依据(倒数第二)** → 交易纪律(核心铁律+6类×4条,置末)。9 决策增强模块按 v2.1 参考模板版式:决策总览 4 卡片、情景预案 5 列表格、凯利左公式右交互双面板、相关性热力图+3 列解读、风险仪表盘 4 指标卡+权益曲线+风险预警框、动态止损分组柱状图+对比表+推荐绿框、交易日志明细表格、信号回测整体面板+R 乘数曲线+明细表+结论 4 列、独立 Excel 复盘模板(openpyxl,4 工作表)。稳健模式:各节构建为独立变量再按序拼装(便于调序),全部 ECharts option 用 `json.dumps(ensure_ascii=False)` 注入(杜绝 f-string 花括号错误),CSS 复用 `assets/decision_enhanced_sample.html` 的 `<style>` 段,产出经 `node --check` + 19 节顺序 + 12 图表容器/初始化一一对应校验。快照现价/日内%/区间由 Twelve Data 真实 K 线末两根计算。修改顶部 `OUT`/`NOW`/`ACC`/`POS`/`KELLY`/`BT`/`OTHER7`/`RECOMMEND5`/`DISCIPLINE6`/`QUOTES`/`SCORES`/`SNAP_EXTRA`/`CB_POLICY`/`CARRY2`/`GEO_RISK`/`MACRO2`/`CAL_PUB`/`CAL_PENDING`/`CAL_MONTH` 等数据块即可复用于任意交易日。**v3.0 全自动 Agent**:`scripts/report_agent_v3.py` 以本 v2.3 模板 HTML 为「界面冻结基准」(复用其 `<style>` 与 19 节 DOM 骨架),通过 `scripts/calc_engine.py` 统一引擎注入实时数据自动生成报告,详见 `references/v3_spec.md`——模板(v2.3 手工填充)与 Agent(v3.0 自动填充)共用同一界面与计算口径,互不冲突。
- **v2.4 微调生成器(2026-09-13)**:`scripts/decision_enhanced_report_v24.py` — 在 **v2.3 版面零改动**前提下接入 `calc_engine.py` 统一引擎(单一事实来源),仅新增 3 个「补充节」(四·补 评分模型子项明细 / 六·补 事件静默自动计算 / 二十、数据一致性校验报告,全部复用既有 CSS 类);AUDJPY 现价统一为日线最后收盘(消除 v2.3 的 110.46967/110.54/110.47 三处不一致);凯利改"最终=min(半凯利,1%风险,用户上限)"并删除「最优仓位 20%」误导;相关性文字由 60 日 Pearson 矩阵实时生成;回测样本<100 禁年化预测;一致性问题自动标红。输出 `今日行情分析_决策增强版_v2.4_<日期>.html/.md` + `交易复盘模板_v2.4.xlsx`(23 节 / 12 图)。
- **v2.4.1 微调生成器(2026-09-13)**:`scripts/decision_enhanced_report_v241.py` — ①建仓判定四项理由逐行起行;②**修复量化雷达图异常**:旧 `quant_block()` 把 Hurst 算在**收益率序列**上(应算价格序列)、Z 用 `(last−mean)/(sd/√n)` 的 **t 统计量**口径(应为 63 日滚动 Z-score),导致 6 轴仅 2 轴有值、多边形被拉成**细长条**;现改用 `quant_metrics.py` 真实计算 + **AUDJPY vs XAUUSD 双多边形 6 轴 0-100 归一化**(年化波动÷30% / 最大回撤÷40% / 下行偏差÷25% / |偏度|÷1.5 / |Z63|÷3 / 日VaR95÷5%);③第⑨节按参考模板重构为「双图(雷达 + 日对数收益分布直方图)+ **6.1** AUDJPY 200 根指标表 + **6.2** XAUUSD 200 根 + CFTC 拥挤度表 + **6.3** 量化结论表」,接入 `cftc_fetch.py --symbol GOLD` 真实持仓(净多 136,771 手 / 净多占总持仓 32.94% / 52 周分位 **89.22%** / 周环比 ΔNet −7,976 手);新增单日 95% 分位风险量化(AUDJPY 0.02 手 ≈ $11.75 = 账户 2.05%;XAUUSD 0.01 手 ≈ $118.80 = 账户 20.69% → 当前账户不可参与黄金)。输出 `..._v2.4.1_<日期>` 系列(23 节 / **13 图**)。
...
# 持仓深度分析报告一键生成（v1.4.0）—— 已有持仓的全维度诊断报告
`python "./scripts/position_report.py" --symbol AUDJPY --direction SELL --lots 0.02 --entry 114.573 --sl 112.583 --tp 109.781 --account 574 --risk-pct 2.0 --out-dir "./output/持仓分析_AUDJPY_2026-09-10"`
...
- **原油**:**EIA v2 API(`scripts/eia_fetch.py`:周度库存 `WCRSTUS1` / 汽油总库存 `WGTSTUS1` / 馏分油库存 `WDISTUS1`(千桶)、WTI `RWTC`/Brent `RBRTE` 现货价(美元/桶),key 需自备,可存于 `scripts/.eia_key`)**、**`scripts/opec_fetch.py`(OPEC MOMR best-effort 直连 + WebFetch 手工取数指引:全球需求增长/非 OPEC 供应/OPEC 产量/ORB 一揽子价)**、IEA(付费 MODS,附 KAPSARC 免费历史替代)、API 补充、**QuantGist API(`scripts/quantgist_fetch.py`:`news/radar` 含 oil-supply/middle-east-risk/opec 主题与 CL/USO 受影响资产、商品 ETF 快照 USO,需 X-API-Key)**。
- **跨市场**:TradingView 图表模板、用户"全球金融日报"JSON(复用 40 品种,免重复采集)。
- **K 线历史(盘面分析用)**:当用户未发送 K 线时,**Twelve Data API(`scripts/kline_fetch.py` + `kline_read.py --fetch`)** 是中国大陆可直连、免翻墙的权威 OHLC 历史源,覆盖外汇/黄金/原油及 15min/1h/4h/1day/1week 五周期,需免费 API Key(环境变量 `TWELVEDATA_API_KEY` 或 `--api-key`)。抓取严格遵循**不编造铁律**:任一周期失败仅报告原因跳过,全部失败则明确告知"无法获取权威 K 线数据"并引导改用本地 CSV/文本,绝不估算价格。历史行情复盘另可参考**用户腾讯 IMA 知识库**中已沉淀的 K 线与复盘数据(引用须标注 `[源: IMA 知识库 | 截至: YYYY-MM-DD]`,不作唯一权威源,以实时 API 为准);详见 `references/data_sources.md` 第 7.7 节。
...
- **期货分析数据源(goldprice.dev / OilPriceAPI / Frankfurter / AllRatesToday)**:`scripts/goldprice_fetch.py` 取 goldprice.dev 现货金价(`XAU-USD-SPOT`,现货端基准);`scripts/oilprice_fetch.py` 取 OilPriceAPI 的 WTI/Brent 实时与历史价(`Authorization: Token`,WTI-Brent 跨品种价差原料);`scripts/frankfurter_fetch.py` 取 ECB 官方日参考汇率(宏观套息/中期汇率方向);`scripts/allratestoday_fetch.py` 取实时银行间中间价(盘中监控)。三者经 `scripts/futures_analysis.py` 的 `pull` 子命令直连聚合——`--kind oil` 实时算 WTI−Brent 价差,`--kind gold` 算黄金期现结构(goldprice.dev 现货 + qveris 现货交叉验证;GC 期货端经 `basis --fut` 手动传入,因 iTick 已移除无免费期货源);亦可用 `basis`/`spread` 子命令纯输入计算(期现结构 contango/backwardation、跨期/跨品种价差与 z-score 偏离)。goldprice.dev 在本构建环境被 Cloudflare 拦截(本机正常),属已知限制。
...
- **EIA key**:需自备 key 并写入 `scripts/.eia_key`(脚本同目录单行纯文本),**仅本地存在、不进入发布包、不写进脚本源码**。读取优先级:`--api-key` > 环境变量 `EIA_API_KEY` > `scripts/.eia_key`。用户重装/迁移 skill 后需重新生成该文件或设置环境变量。
- **FRED key**(模块一 宏观利率地基,`scripts/fred_fetch.py`):需自备 key 并写入 `scripts/.fred_key`(脚本同目录单行纯文本,本地文件、不进 zip);读取优先级:`--api-key` > 环境变量 `FRED_API_KEY` > `scripts/.fred_key`。免费注册 https://fredaccount.stlouisfed.org/apikeys。
...
- **AllRatesToday key**(实时银行间中间价,`scripts/allratestoday_fetch.py`):Bearer Token 存于 `scripts/.art_key`(单行纯文本,本地文件、不进 zip);读取优先级:`--api-key` > 环境变量 `ART_KEY` > `scripts/.art_key`。注册 https://allratestoday.com 获取(免费层额度有限,429/402 时优雅提示升级)。
...
- **qveris MCP token**(金融数据市场:金银/外汇/宏观,`scripts/qveris_fetch.py`):Bearer Token 存于 `~/.workbuddy/mcp.json` 的 `mcpServers.qveris.headers.Authorization`(WorkBuddy MCP 连接器用,已验证可用),亦支持 `scripts/.qveris_key`(脚本用,单行纯文本,本地文件、不进 zip)与 `--api-key` / 环境变量 `QVERIS_API_KEY`;`qveris_fetch.py` 读取优先级:`--api-key` > `QVERIS_API_KEY` > `scripts/.qveris_key` > `~/.workbuddy/mcp.json`。qveris **按调用计费**(2026-09 实测:金银现货≈9.55、外汇日线≈1、外汇实时≈2.81 credits/次,账户额度 1000),批量扫描前用 `qveris_fetch.py credits` 确认余量;不硬编码进脚本与 zip。详见 `references/data_sources.md` 第 7.16 节。
- **原则**:真实密钥(第三方 API key)一律不落盘于可分发产物(zip/源码),避免泄露;本地便利文件(`.eia_key` / `.fred_key` / `.td_key` / `.jin10_key` / `.art_key` / `.goldprice_key` / `.oilprice_key` / `.qveris_key`)由用户自行保管,打包/发布时强制排除。如怀疑泄露,立即到对应平台吊销并换新 key。

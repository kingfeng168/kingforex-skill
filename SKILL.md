...
- **`assets/position_report_template.html` — v2.0 当前 · 本技能唯一批准的分析报告模板(强制套用)**:九大节固定结构(纪律置末),真实 K 线 + 量化雷达 + 跨市场柱图 + 4 线 markLine,深空蓝霓虹暗色版面。**v2.0 重大升级(融合博士「决策增强版」)**:在九大节骨架基础上新增 9 个决策增强模块——①今日决策总览(账户/信号质量/凯利仓位/最大风险 + 一句话结论) ②8 标的综合评分卡(宏观25%+技术30%+量化25%+情绪20% 四维加权,≥75 可建仓) ③概率加权情景预案(三情景 + 概率,只准备不预测) ④凯利仓位计算器(公式 + 回测参数 + 交互) ⑤品种相关性矩阵(60 日热力图 + 组合诊断) ⑥账户风险仪表盘(权益曲线 + 风险预警) ⑦动态止损方案对比(ATR/结构/移动 三方案) ⑧交易决策日志(当前持仓 + 复盘模板) ⑨信号历史回测(策略表现 + R 乘数资金曲线 + 信号明细)。并完整保留 v1.4.3 修复:雷达图 radius 55%/center 54% 防溢出、关键位表格化(品种/关键位/类型/作用)、最终推荐必须附「抉择依据」。**所有分析报告(持仓诊断 / 多品种计划 / 专项分析)强制以该骨架输出**——持仓类用 `scripts/position_report.py` 一键生成,非持仓类按 `references/position_report.md` 第 0 节适配第①节后手动填充;完整决策增强实战样本见 `assets/decision_enhanced_sample.html`(2026-09-11 八标的 + AUDJPY 持仓,含全部 9 模块)。
